...
def city_search_parallel(city):
    ...
    business_ids = [business['id'] for business in json_response['businesses']]
...
def business_info_concurrent(ids):
    ...
    jobs=[]
    for b_id in ids:
        t = threading.Thread(target=business_info, args=(b_id,))
        jobs.append(t)
        t.start()
    for t in jobs:
        t.join()
...
def scrape_parallel_concurrent(pool_size):
    ...
    city_outputs = pool.map(city_search_parallel, cities)
    return city_outputs
